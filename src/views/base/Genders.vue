<template>
  <div>
    <CCard>
      <CCardHeader>
        <slot name="header"> <CIcon name="cil-grid" /> Genders </slot>
      </CCardHeader>
      <CCardBody>
        <CDataTable
          :items="genders"
    ...
    <!-- modal for delete -->
    <CModal title="Delete Gender" :show.sync="deleteModal">
...
export default {
  ...
  methods: {
    ...
    saveUpdatedGender(myGender) {
      axios
        .put(
          `http://localhost:8080/gender/save/${myGender.gender_id}`,
          myGender
        )
        .then((response) => {})
        .catch((e) => {
    ...
    deleteGender(myGenderDelete) {
      axios
        .delete(
          `http://localhost:8080/gender/delete/${myGenderDelete.gender_id}`
        )
        .then((response) => {})
        .catch((e) => {
          console.log(e);
